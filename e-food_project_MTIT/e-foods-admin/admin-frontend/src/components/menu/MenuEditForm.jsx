            ...
            <div className="bg-[#1e293b] w-full max-w-3xl p-8 rounded-3xl relative shadow-2xl overflow-y-auto max-h-[90vh] border border-slate-800">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-2 gap-6">
                    <div className="space-y-4 col-span-2 md:col-span-1">
                        ...
                        <Input icon={<Utensils className="w-4 h-4" />} name="name" value={formData.name} onChange={handleChange} />
                        <Input icon={<List className="w-4 h-4" />} name="category" value={formData.category} onChange={handleChange} />
                        <div className="grid grid-cols-2 gap-4">
                            <Input icon={<Coins className="w-4 h-4" />} name="price" value={formData.price} type="number" onChange={handleChange} />
                            <Input icon={<Percent className="w-4 h-4" />} name="discount" value={formData.discount} type="number" onChange={handleChange} />
                        </div>
                        <Input icon={<Clock className="w-4 h-4" />} name="prepTime" value={formData.prepTime} type="number" onChange={handleChange} />
                    </div>

                    <div className="space-y-4 col-span-2 md:col-span-1">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1">Inventory Management</label>
                        <Input icon={<Boxes className="w-4 h-4" />} name="stock" value={formData.stock} type="number" onChange={handleChange} />
                        <Input icon={<Boxes className="w-4 h-4" />} name="lowStockThreshold" value={formData.lowStockThreshold} type="number" onChange={handleChange} />
                        <Input icon={<Tag className="w-4 h-4" />} name="tags" value={formData.tags} onChange={handleChange} />
                        <Input icon={<Boxes className="w-4 h-4" />} name="ingredients" value={formData.ingredients} onChange={handleChange} />
                    </div>

                    <div className="col-span-2">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2">Description</label>
                        <textarea
                            name="description"
                            rows="3"
                    ...
                    <div className="col-span-2 border border-slate-800 p-6 rounded-2xl bg-[#0f172a]/30">
                        ...
                        {formData.sizes.map((s, i) => (
                            <div key={i} className="flex gap-4 mb-3 items-center">
                                <input className="flex-1 bg-[#0f172a] border border-slate-800 rounded-xl p-3 text-white font-bold outline-none" value={s.size} onChange={e => handleSizeChange(i, 'size', e.target.value)} />
                                <input type="number" className="flex-1 bg-[#0f172a] border border-slate-800 rounded-xl p-3 text-white font-bold outline-none" value={s.price} onChange={e => handleSizeChange(i, 'price', e.target.value)} />
                    ...
                    <div className="col-span-2">
                        <label className="block text-[10px] font-black text-slate-500 uppercase tracking-widest ml-1 mb-2">Item Image</label>
                            ...
                            {imagePreview ? (
                                <div className="relative h-40 w-full">
                                    ...
                                    <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center rounded-xl">
                                        <span className="text-white text-xs font-black uppercase tracking-widest">Change Visual</span>
                                    </div>
                                </div>
                            ) : (
                                <div className="py-4">
                                    <ImagePlus className="w-10 h-10 text-slate-600 mx-auto mb-2" />
                                    <p className="text-xs font-bold text-slate-500 uppercase tracking-tighter">Update visual assets</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="col-span-2 flex items-center gap-3 p-4 bg-[#0f172a]/30 rounded-2xl border border-slate-800">
                        ...
                        <label className="text-sm font-bold text-slate-300">Visible for Consumers</label>
                    </div>

                    <div className="col-span-2 flex justify-end gap-4 mt-4">
                        <button type="button" onClick={onClose} className="px-8 py-3 text-slate-400 font-bold hover:text-white transition-colors">Discard</button>
                        <button type="submit" className="bg-orange-500 hover:bg-orange-600 text-white px-10 py-3 rounded-2xl font-black uppercase tracking-widest shadow-xl shadow-orange-500/20 transition-all active:scale-95">Update Listing</button>
                    </div>
                </form>
            </div>
        </div>
